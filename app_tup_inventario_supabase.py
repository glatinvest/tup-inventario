...
def estrai_fornitore(testo, nome_file):
    ...
    for r in righe[:50]:
        ...
        if re.search(r"\b(srl|s\.r\.l\.|spa|s\.p\.a\.|sas|snc|ditta|azienda|panzini|nasta|marr)\b", rl):
            return r[:100]
        ...
        if not righe:
            ...
            if st.button("Salva in anagrafica"):
                ...
                if referenze.empty:
                    ...
                    referenze["match"] = referenze["referenza"].apply(nome_match)
                ...
                for _, row in df_da_salvare.iterrows():
                    ...
                    match = nome_match(nome)

                    if not referenze.empty and match in referenze["match"].values:
                        existing = referenze[referenze["match"] == match].iloc[0]
                        update_row("referenze", existing["id"], {
                ...
                st.success(f"Salvato. Nuove referenze: {inserite}. Aggiornate: {aggiornate}.")
...
# =========================
    ...
    if referenze.empty:
        ...
        store_tabs = st.tabs(PUNTI_VENDITA)
        ...
        for tab, store in zip(store_tabs, PUNTI_VENDITA):
            with tab:
...
# =========================
    ...
    if referenze.empty:
        ...
        with st.form("form_trasferimento"):
            ...
            with col1:
                da_store = st.selectbox("Da", PUNTI_VENDITA, index=0)
            with col2:
